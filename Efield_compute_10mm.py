...
with st.sidebar:
    ...
    fig_height_plotly = st.slider("Plotly fig height", min_value=100, max_value=1000, value=300)
    
    col1, col2 = st.columns(2)  
    with col1:
        matplot_axis_label_size = st.slider("MPL Axis label size", min_value=1, max_value=20, value=10)
        matplot_figure_width = st.slider("MPL fig width", min_value=1, max_value=20, value=6)
        ...
        color_map = st.selectbox(
            ...
            index=1)
    with col2:
        matplot_tick_label_size = st.slider("MPL Tick label size", min_value=1, max_value=20, value=10)
        matplot_figure_height = st.slider("MPL fig height", min_value=1, max_value=20, value=4)
...
with st.sidebar:
    ...
    with col2:
        d = st.number_input("Path Length (mm)", value=10.0)
        r41 = st.number_input("r41 (1e-12 m/V)", value=5.5)
        alpha = alpha(wavelength, n0, d, r41)
        E_ref = E_ref(wavelength, n0, d, r41)
    st.write(f"$E_{{ref}} = {E_ref:.2e}$ V/m")
    st.write(f"$\\alpha = \\frac{{\\pi}}{{2 E_{{ref}}}} = {np.pi / (2 * E_ref):.2e}$ m/V")
    st.caption(
    ...
    for uploaded_calib_file in uploaded_calib_files:
        ...
        with st.expander(f"Plot {uploaded_calib_file.name}"):
            ...
            calib_fig = heatmap_plot_with_bounding_box(
                ...
                fig_height=fig_height_plotly, bounding_box=bounding_box
            )
            ...
            if calib_img_arrays:
                ...
                if calculate_E_field and perform_T_normalization:
                    ...
                    if bounding_box and row_avg_E_field:
                        ...
                        ax.set_ylabel('Row-wise average E-field (V/m)', fontsize=matplot_axis_label_size)
...
col1, col2 = st.columns(2)
with col1: top_edge_label = st.text_input("Top edge label", value="cathode", key="top_edge_label")
with col2: bottom_edge_label = st.text_input("Bottom edge label", value="anode", key="bottom_edge_label")
...
with st.expander("Row-wise Average E-field", expanded=True):
    if row_avg_E_field_arrays:
        ...
        long_text = (
            "$T = \\frac{I_{bias}-I_{cross}}{I_{parallel}-I_{off}} = \\sin^2(\\alpha E)$ => $E=\\arcsin(\sqrt{T})/\\alpha$\n"
            # "$T$ is the normalized transmission\n"
            # "$E$ is the vertical component of the electric field\n"
            "$\\alpha=\\frac{\\sqrt{3}\\pi}{2}\\cdot\\frac{d n_o^3 r_{41}}{\\lambda}$, Pockels parameter\n"
            f"$d={d}$ mm, optical path length\n"
            f"$r_{{41}}={r41} \\times 10^{{-12}}$ m/V, electro-optic coefficient\n" 
            f"$n_o={n0}$, refractive index\n"
            f"$\\lambda={wavelength}$ nm, wavelength of the light"
        )
        ...
        # Add shaded margins of 5 pixels on left and right
        margin_width = 5
        ax.axvspan(y0, y0 + margin_width, color='orange', alpha=0.2, label=top_edge_label)  # Left margin
        ax.axvspan(y1 - margin_width, y1, color='green', alpha=0.2, label=bottom_edge_label)  # Right margin
        ax.set_ylim(bottom=0)  # Set minimum y value to 0 while keeping auto maximum
        ax.set_xlabel('Row position (pixels)', fontsize=matplot_axis_label_size)
        ax.set_ylabel('Row-wise Average E-field (V/m)', fontsize=matplot_axis_label_size)
        ax.set_title(f'{sensor_id} - E-field profile', fontsize=matplot_axis_label_size)
        ax.grid(True)
        ax.ticklabel_format(axis='y', style='sci', scilimits=(0,0))
        ax.tick_params(axis='both', which='major', labelsize=matplot_tick_label_size)
...
with st.expander("Efield Matplotlib Plots", expanded=True):
    if E_field_arrays:
        ...
        for ax in st.session_state.mat_fig.axes:
            ax.text(bounding_box[0], bounding_box[1]-5, top_edge_label, 
                    color=box_color, fontsize=matplot_tick_label_size,
                    horizontalalignment='left')
            ax.text(bounding_box[0], bounding_box[3]+7, bottom_edge_label,
                    color=box_color, fontsize=matplot_tick_label_size, 
                    horizontalalignment='left')
        st.session_state.mat_fig.suptitle(f"{sensor_id} - E-field heatmaps {color_range_radio}-scale color range", fontsize=15, y=1.05)
