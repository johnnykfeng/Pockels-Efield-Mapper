...
import numpy as np
from image_process import (png_to_array, crop_image, 
                           find_dead_pixels, find_bright_pixels, impute_bad_pixels, 
                           cap_array, remove_low_value_pixels)
from plotting_modules import create_plotly_figure, plot_histogram, image_array_statistics
...
# @st.cache_data
def plot_image_with_color_slider(image_array, title, color_map, global_range_pctl, pctl_color_range=False, return_fig=False):
    if pctl_color_range:
        col1, col2 = st.columns(2)
        with col1:
            min_pctl = st.number_input("Min percentile", min_value=0.0, max_value=100.0, value=5.0, key=f"{title}_min_pctl")
        with col2:
            max_pctl = st.number_input("Max percentile", min_value=0.0, max_value=100.0, value=95.0, key=f"{title}_max_pctl")
...
if uploaded_calib_files:
    for uploaded_calib_file in uploaded_calib_files:
        ...
        with st.expander(f"Plot {uploaded_calib_file.name}"):
            plot_image_with_color_slider(img_array, f"{uploaded_calib_file.name}", color_map, global_range_pctl)
...
if uploaded_data_files:
    for uploaded_data_file in uploaded_data_files:
        ...
        with st.expander(f"Plot {uploaded_data_file.name}"):
            plot_image_with_color_slider(img_array, f"{uploaded_data_file.name}", color_map, global_range_pctl)
            if calib_img_arrays:
                ...
                do_cap_array = st.checkbox("Normalize transmission (0<T<1)", value=True, key=f"do_cap_array_{uploaded_data_file.name}")
                if do_cap_array:
                    T_array = cap_array(T_array, 0, 1.0)
                plot_image_with_color_slider(T_array, f"Transmission_{uploaded_data_file.name}", color_map, global_range_pctl, pctl_color_range=True)
                ...
                if do_E_field and do_cap_array:
                    ...
                    plot_image_with_color_slider(E_field, f"E-field_{uploaded_data_file.name}", color_map, global_range_pctl, pctl_color_range=True)
